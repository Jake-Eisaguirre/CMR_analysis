...
librarian::shelf(tidyverse, RPostgres, DBI, unmarked, here, lubridate, kableExtra, GGally)
...
area <- nmix_raw_data %>% 
  ...
  select(!c(site)) %>% 
  mutate(area_sqr_m = (area_sqr_m - min(area_sqr_m))/ (max(area_sqr_m) - min(area_sqr_m))) %>% 
  as.data.frame()
...
f_duration_mins <- n_mix_obs_cov_mid %>% 
  ...
  mutate(duration_minutes = (duration_minutes - min(duration_minutes))/ (max(duration_minutes) - min(duration_minutes))) %>% 
...
f_air <- n_mix_obs_cov_mid %>% 
  ...
  mutate(air_temp_c = (air_temp_c - min(air_temp_c))/ (max(air_temp_c) - min(air_temp_c))) %>% 
...
fm1_bf3 <- pcount( ~ survey_minutes  + air_temp + sky + wind + rh ~ area_sqr_m, 
               data = bf_umf_3, 
               engine = "R",
