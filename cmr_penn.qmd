...
librarian::shelf(tidyverse, RPostgres, DBI, unmarked, here, lubridate, kableExtra, GGally, MuMIn, AHMbook, DMwR)
...
# exlude: water_temp
ggpairs(n_mix_obs_cov_mid, progress = F, columns = 3:9)
...
# ~ air_temp ~ 1 best model?
dredge(fm1_bf3, beta = "none")

best_bf3 <- pcount( ~ air_temp ~ 1, 
               data = bf_umf_3, 
               engine = "R",
               control=list(trace=T, REPORT=5))
...
# residual check
bf3_res <- residuals(best_bf3) %>% 
  as.data.frame()

qqnorm(bf3_res$visit_1)
qqnorm(bf3_res$visit_2)
qqnorm(bf3_res$visit_3)

lambad.hat <- predict(best_bf3, type = "state")
p.hat <- predict(best_bf3, type = "det")



pred_data <- data.frame(air_temp = seq(-0.761, 1.55, , 10))

predicted_air_temp <- predict(best_bf3, type = "state", newdata = pred_data, append = T)
...
fm1_bf2 <- pcount( ~ survey_minutes  + air_temp + rh + sky + wind ~ area_sqr_m, 
               data = bf_umf_2, 
               engine = "R",
               control=list(trace=T, REPORT=5))
...
crPiFun <- function(p) {
    ...
    cbind("100" = p1 * (1-p2) * (1-p3),    #1: 100    
          ...
          "111" = p1 * p2 * (1-p3),        #4: 110
          "110" = (1-p1) * p2 * p3         #5: 011
